...
<body class="skin-blue">
	...
	<section class="content">
			...
			<div class="col-xs-12">
				...
				<div class="box box-primary">
						...
						<div class="col-xs-4 form-group">
							...
							<select name="Platform" id="Platform" class="form-control col-xs-4">
						...
						<div class="col-xs-4 form-group">
							<label>Status</label>
							<select name="Status" id="Status"  class="form-control col-xs-4">
											<option value="">Select</option>
						...
						<div class="col-xs-4 form-group">
							...
							<input type="text"  maxlength="10" class="form-control col-xs-4 fleft" name="SearchDate" id="SearchDate" title="Select Date" value="<?php if(isset($_SESSION['tuplit_sess_user_registerdate']) && $_SESSION['tuplit_sess_user_registerdate'] != '') echo date('m/d/Y',strtotime($_SESSION['tuplit_sess_user_registerdate'])); else echo '';?>" > &nbsp;&nbsp;(mm/dd/yyyy)
						</div>
		...
		<?php if(isset($msg) && $msg != '') { ?>
		 <div class="row">
               <div align="center" class="alert <?php  echo $class;  ?> alert-dismissable col-xs-4"><i class="fa fa-check"></i>  <?php echo $msg; ?></div>
		 </div>	
		<?php } ?>
                   ...
                   <div class="box">
                       <div class="box-body table-responsive no-padding">
                           ...
                           <table class="table table-hover">
                               <tr>
                                  	<th align="center" width="1%" style="text-align:center"><input type="checkbox" onclick="checkAllDelete('UserListForm');"  name="checkAll" id="checkAll"/></th>
							...
							<tr>
								...
								<td>
									<div class="relative user_list"> 
									<div class="row"> 
										<?php if(isset($image_path) && $image_path != ''){ ?><div class="col-xs-3"><a <?php if(isset($image_path) && basename($image_path) != "no_user.jpeg") { ?>href="<?php echo $original_path; ?>" class="user_image_pop_up" title="View Photo" <?php } ?> ><img width="36" height="36" align="top" class="img_border" src="<?php echo $image_path;?>" ></a></div> <?php } ?>
										<div class="col-xs-12"> 
										<i class="fa fa-fw fa-user"></i> <?php if(isset($value->FirstName) && $value->FirstName != ''){ echo $value->FirstName.' '; } ?>
										<?php if(isset($value->LastName) && $value->LastName != ''){ echo $value->LastName; } ?><br>
										
										<i class="fa fa-fw fa-envelope"></i> <?php if(isset($value->Email) && $value->Email != '' ){ echo $value->Email;} ?><br>
										
										<i class="fa fa-fw fa-phone"></i> <?php if(isset($value->CellNumber) && $value->CellNumber != ''){ echo $value->CellNumber; } ?><br>
										
										<i class="fa fa-fw fa-facebook"></i> <?php if(isset($value->FBId) && $value->FBId != ''){ echo $value->FBId; }else echo '-';?><br>
										
										<i class="fa fa-fw fa-google-plus"></i> <?php if(isset($value->GooglePlusId) && $value->GooglePlusId != ''){ echo $value->GooglePlusId; }else echo '-';?><br>
										</div>
									</div><!-- /row -->
									<div class="row-actions">
										<?php if($value->Status == 1) { ?><a class="active_icon" onclick="javascript:return confirm('Are you sure want to change the status?')" href="UserList?status=2&editId=<?php if(isset($value->id) && $value->id != '') echo $value->id;?>" alt="Click to Inactive" title="Click to Inactive"><i class="fa fa-user "></i></a><?php } else { ?><a class="inactive_icon" onclick="javascript:return confirm('Are you sure you want to change the status?')" title="Click to Active" alt="Click to Active" href="UserList?status=1&editId=<?php if(isset($value->id) && $value->id != '') echo $value->id;?>"><i class="fa fa-user "></i></a><?php } ?>
																						
										<a href="UserManage?editId=<?php if(isset($value->id) && $value->id != '') echo $value->id; ?>" title="Edit" alt="Edit" class="edit"><i class="fa fa-edit "></i></a>
		
										<a href="UserDetail?viewId=<?php if(isset($value->id) && $value->id != '') echo $value->id; ?>" title="View" alt="View" class="view"><i class="fa fa-search "></i></a>	
												
										<a onclick="<?php if(isset($value->hash_count) && $value->hash_count > 0 ) { ?>alert('Sorry! you can not delete this user'); return false; <?php } else { ?>javascript:return confirm('Are you sure to delete?') <?php } ?>" href="UserList?delId=<?php if(isset($value->id) && $value->id != '') echo $value->id;?>" title="Delete" alt="Delete" class="delete"><i class="fa fa-trash-o "></i></a>
									</div>
									</div>
								</td>
								<td>
									<i class="fa fa-fw fa-map-marker"></i> <?php if(isset($value->Location) && $value->Location != ''){ echo $value->Location; }else echo '-';?>
									
									 , <?php if(isset($value->Country) && $value->Country != ''){ echo $value->Country; }else echo '-';?>
									
									 , <?php if(isset($value->ZipCode) && $value->ZipCode != ''){ echo $value->ZipCode; }else echo '-';?><br>
									
									<i class="fa fa-fw fa-mobile"></i> <?php if(isset($value->Platform) && $value->Platform != ''){ echo $platformArray[$value->Platform]; }else echo '-';?><br>
								...
								<td>
									<i class="fa fa-fw fa-map-marker"></i> <?php if(isset($value->Location) && $value->Location != ''){ echo $value->Location; }else echo '-';?><br>
									
									<i class="fa fa-fw fa-flag-o"></i> <?php if(isset($value->Country) && $value->Country != ''){ echo $value->Country; }else echo '-';?><br>
									
									<i class="fa fa-fw fa-barcode"></i> <?php if(isset($value->ZipCode) && $value->ZipCode != ''){ echo $value->ZipCode; }else echo '-';?><br>
								...
								<td></td>	
								<td></td>	 -->
							<!--	<td><?php //if(isset($value->CellNumber) && $value->CellNumber != ''){ echo $value->CellNumber; }else echo '-';?></td>	-->
								<!-- <td></td>	
								<td></td> -->

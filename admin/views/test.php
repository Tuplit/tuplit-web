...
<body class="skin-blue" onload="">
	...
	<section class="content">
		<!--<div class="row">
			<div class="col-xs-12">
				<form name="search_merchant" action="ProductList" method="post">
		...
		<div class="row paging">
			...
			<div class="col-xs-10">
				...
				<?php if(is_array($productListResult) && count($productListResult) > 0 ) {
						pagingControlLatest($tot_rec,'ProductList'); ?>
				<?php }?>
				</div>
			</div>
		 </div>	
...
<!--<a href="ProductDetail?viewId=<?php //if(isset($value->id) && $value->id != '') echo $value->id; ?>" title="View" alt="View" class="view"><i class="fa fa-search "></i></a>		
								...
								<?
									$image_path = '';
									$photo = $value->Photo;
									$image_path = SITE_PATH.'/Refer/site_source/no_photo2.jpg';
...
<!--<div class="row">
					...
					<?php } else { ?>	
						<div class="alert alert-danger alert-dismissable col-xs-4 "><i class="fa fa-warning"></i> No Products found for this merchant</div> 
					<?php } ?>	
               </div>
           </div>

 ...
 <div class="row table-responsive no-margin" id="pro_list">
	...
	<form  id="productAnalytics" name="productAnalytics" action="#" method="post">
		...
		<table class="table table-hover" border="0" >
	         <tr>
				<th style="text-align:left"><a onclick="sortTable('Name');" id="Name" name="Name" style="cursor:pointer;">Name<img id="name_bg" src="<?php echo  ADMIN_IMAGE_PATH.'no_sort.gif'?>"></a></th>
				<th style="text-align:center" colspan="2"><a onclick="sortTable('TotalPrice');" id="TotalPrice" name="TotalPrice" style="cursor:pointer;">Sales<img id="sales_bg" src="<?php echo  ADMIN_IMAGE_PATH.'no_sort.gif'?>"></a></th>
				<th style="text-align:center"><a onclick="sortTable('TotalQuantity');" id="TotalQuantity" name="TotalQuantity" style="cursor:pointer;">Quantity<img id="quantity_bg" src="<?php echo  ADMIN_IMAGE_PATH.'no_sort.gif'?>"></a></th>
			</tr>
			<th>
	        ...
	        <?php
			$total_price = $total_quantity = $average	= $averageTransaction = 0;
		  	foreach($product_array as $key=>$value){
				$total_price	=	number_format((float)$value->TotalPrice, 2, '.', ',');
				$total_quantity	=	$value->TotalQuantity;
				if($total_quantity	> 0)
					$averageTransaction	=	$value->TotalPrice/$total_quantity;
				$average			=	number_format((float)$averageTransaction, 2, '.', ',');
				$count += 1;?>

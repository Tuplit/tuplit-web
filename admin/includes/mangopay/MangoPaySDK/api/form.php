...
if (isset($_POST['_postback']) && $_POST['_postback'] == '1') {
    ...
    try {
        ...
        $api->Config->ClientId = 'tuplit-2014';
        $api->Config->ClientPassword = 'reG1VZFeXSvxp6AcXKWKaL9qWmYQi9fMyfizMww8BgTzmgrmBK';
        ...
        if (!isset($apiResult))
        {
            switch ($operation) {
                ...
                case 'ListSubEntity':
                    ...
                    if (isset($filterName) && $filterName != "")
                        $filter = HtmlHelper::getEntity($filterName);
						echo '<pre>===>';print_r($pagination);echo '<===</pre>';echo '<pre>=filter==>';print_r($filter);echo '<===</pre>';

...
<body class="skin-blue fixed drag_box" onload="fieldfocus('ItemName');">
		...
		<div class="col-lg-12 box-center">	
			<?php if(empty($merchantInfo['MangoPayUniqueId'])){?>
				<div align="center" class="alert alert-danger alert-dismissable  col-lg-5 col-sm-7  col-md-5 col-xs-12"><i class="fa fa-warning"></i>&nbsp;&nbsp;Please connect with MangoPay in Settings to add products.</div>
				...
				<div class="box-body">
					...
					<?php if(isset($productList) && !empty($productList)) {	 
							...
							<div class="col-xs-4 text-right pad"><a href="Product?show=0&add=<?php echo $key; ?>" class="newWindow"><i class="fa fa-plus"></i> Add Item</a></div>
								<ul  style="min-height:50px;" class="row  no-padding no-margin col-xs-12 clear draggableList" id="sortable_<?php echo $key; ?>">
									<?php 
									$value = subval_sort($value,'Ordering');
									foreach($value as $key1=>$value1 ) {
										if($value1["ProductId"]!= ''){ ?>	
											<li class="col-xs-12 col-md-3 col-sm-4 col-lg-2 ui-state-default">
												<div >
												<div id="<?php echo $value1["ProductId"];?>" class="small-box panel-heading">
													<a href="<?php echo $value1['Photo'];?>" class="Product_fancybox" title="<?php echo ucfirst($value1['ItemName']);?>">
														<img height="100" width="100" src="<?php echo $value1['Photo']; ?>" alt=""><br/>
		...
		<?php	if(isset($productList) && count($productList) > 0) { foreach($productList as $key=>$value) {  ?>	
				$("#sortable_<?php echo $key; ?>").sortable({
					...
					update: function() {idsarray	=	new Array();
							i			=	0;
							//cat_id		=	'';
							$('.paneldragging', "#sortable_<?php echo $key; ?>").each(function(index, elem) {
								 var $listItem 	= $(elem);
				...
				$( "#sortable_<?php echo $key; ?>" ).disableSelection();
		<?php } } ?>
